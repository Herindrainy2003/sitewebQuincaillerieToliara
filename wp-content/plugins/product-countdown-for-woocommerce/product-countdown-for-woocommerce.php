...
final class ProWC_Product_Countdown {
	...
	function action_links( $links ) {
		...
		if ( 'product-countdown-for-woocommerce.php' === basename( __FILE__ ) ) {
			$custom_links[] = '<a href="https://prowcplugins.com/downloads/product-time-countdown-for-woocommerce/">' .
				__( 'Unlock All', PCFWC_TEXTDOMAIN ) . '</a>';
		}
	...
	function prowc_product_countdown_check_version() {
		if ( version_compare( $this->version, '1.5.0', '>' )) {
			...
			$old_keys_pm = $wpdb->get_results( "SELECT *  FROM `". $table_postmeta ."` WHERE `meta_key` LIKE '%_alg%' ");
			if(!empty($old_keys_pm)) {
					foreach($old_keys_pm as $val_pm) {
						$new_key_pm = str_replace("_alg","prowc", $val_pm->meta_key);
						$wpdb->query( $wpdb->prepare( "UPDATE $table_postmeta SET meta_key = %s WHERE meta_key = %s", $new_key_pm, $val_pm->meta_key ) );
					}
			}
		}
	}
	...
	function fail_load() {
	    ...
	    if ($this->prowc_PC_installed()) {
	        ...
	        $message = '<p><strong>' . esc_html__('Product Time Countdown for WooCommerce', PCFWC_TEXTDOMAIN) . '</strong>' . esc_html__(' plugin is not working because you need to activate the Woocommerce plugin.', PCFWC_TEXTDOMAIN) . '</p>';
	        $message .= '<p>' . sprintf('<a href="%s" class="button-primary">%s</a>', $activation_url, __('Activate Woocommerce Now', PCFWC_TEXTDOMAIN)) . '</p>';
	    } else {
	        ...
	        $message = '<p><strong>' . esc_html__('Product Time Countdown for WooCommerce', PCFWC_TEXTDOMAIN) . '</strong>' . esc_html__(' plugin is not working because you need to install the WooCoomerce plugin', PCFWC_TEXTDOMAIN) . '</p>';
	        $message .= '<p>' . sprintf('<a href="%s" class="button-primary">%s</a>', $install_url, __('Install WooCommerce Now', PCFWC_TEXTDOMAIN)) . '</p>';
	    }
	...
	function prowc_product_countdown_admin_style(){
		wp_enqueue_style('prowc-wc-pc-style', PCFWC_PC_URL . '/includes/css/admin-style.css');
	...
	function prowc_product_countdown_admin_upgrade_notice() {
		...
		if (date('Y-m-d') >= $rDate && !get_option('prowc_product_countdown_plugin_notice_dismissed')) {
			?>
			<div class="notice is-dismissible prowc_product_countdown_prowc_notice">
				<div class="prowc_product_countdown_wrap">
					<div class="prowc_product_countdown_gravatar">
						<img alt="" src="<?php echo PCFWC_PC_URL . '/includes/img/prowc_logo.png' ?>">
					</div>
					<div class="prowc_product_countdown_authorname">
						<div class="notice_texts">
							<a href="<?php echo esc_url('https://prowcplugins.com/downloads/product-time-countdown-for-woocommerce/?utm_source=product-time-countdown-for-woocommerce&utm_medium=referral&utm_campaign=settings'); ?>" target="_blank"><?php esc_html_e('Upgrade Product Time Countdown for WooCommerce', PCFWC_TEXTDOMAIN); ?> </a> <?php esc_html_e('to get additional features, security, and support. ', PCFWC_TEXTDOMAIN); ?> <strong><?php esc_html_e('Get 20% OFF', PCFWC_TEXTDOMAIN); ?></strong><?php esc_html_e(' your upgrade, use coupon code', PCFWC_TEXTDOMAIN); ?> <strong><?php esc_html_e('WP20', PCFWC_TEXTDOMAIN); ?></strong>
						</div>
						<div class="prowc_product_countdown_desc">
							<div class="notice_button">
								<a class="prowc_product_countdown_button button-primary" href="<?php echo esc_url('https://prowcplugins.com/downloads/product-time-countdown-for-woocommerce/?utm_source=product-time-countdown-for-woocommerce&utm_medium=referral&utm_campaign=settings'); ?>" target="_blank"><?php echo _e('Buy Now', PCFWC_TEXTDOMAIN); ?></a>
								<a href="?prowc-pc-plugin-remindlater"><?php echo _e('Remind me later', PCFWC_TEXTDOMAIN); ?></a>
								<a href="?prowc-pc-plugin-dismissed"><?php echo _e('Dismiss Notice', PCFWC_TEXTDOMAIN); ?></a>
							</div>
						</div>
					</div>
	...
	function prowc_product_countdown_plugin_notice_remindlater() {
		...
		if (isset($_GET['prowc-pc-plugin-remindlater'])) {
	...
	function prowc_product_countdown_admin_review_notice() {
		...
		if (date('Y-m-d') >= $activationDate && date('Y-m-d') >= $rating_rDate && !get_option('prowc_pcfwc_notice_dismissed_rating')) {
			?>
				<div class="notice notice-info  is-dismissible">
					<p><?php  printf( __( 'How are you liking the %s?', PCFWC_TEXTDOMAIN ), esc_html( $plugin_name ) ); ?></p>
					<div class="pcfwc_starts_main_div">
						...
						<div class="notice_button">
							<a href="?prowc-wc-pcfwc-plugin-remindlater-rating" class="button-secondary" ><?php _e('Remind me later', PCFWC_TEXTDOMAIN); ?></a>
							<a href="?prowc-wc-pcfwc-plugin-dismissed-rating" class="button-secondary" ><?php _e('Dismiss Notice', PCFWC_TEXTDOMAIN); ?></a>
						</div>
					</div>
				</div>
			<?php
		...
		if ($rDater != "")
			if (date('Y-m-d') >= $rDater && $algtrack && !get_option('prowc_product_countdown_plugin_notice_alreadydid')) {
			?>
			<div class="notice is-dismissible prowc_product_countdown_prowc_notice">
				<div class="prowc_product_countdown_wrap">
					<div class="prowc_product_countdown_gravatar">
					...
					<div class="prowc_product_countdown_authorname">
						<div class="notice_texts">
							<strong><?php esc_html_e('Are you enjoying Product Time Countdown for WooCommerce?', PCFWC_TEXTDOMAIN); ?></strong>
						</div>
						<div class="prowc_product_countdown_desc">
							<div class="notice_button">
								<button class="prowc_product_countdown_button button-primary prowc_product_countdown_yes"><?php echo _e('Yes!', PCFWC_TEXTDOMAIN); ?></button>
								<a class="prowc_product_countdown_button button action" href="?prowc-wc-pc-plugin-alreadydid"><?php echo _e('Not Really!', PCFWC_TEXTDOMAIN); ?></a>
							</div>
						</div>
					</div>
					<div class="clearfix"></div>
				...
				<div class="prowc_product_countdown_prowc_notice_review_yes">
					<div class="notice_texts">
						<?php esc_html_e('That\'s awesome! Could you please do me a BIG favor and give it 5-star rating on WordPress to help us spread the word and boost our motivation?' , PCFWC_TEXTDOMAIN); ?>
					</div>
					<div class="prowc_product_countdown_desc">
						<div class="notice_button">
							<a class="prowc_product_countdown_button button-primary" href="<?php echo esc_url('https://wordpress.org/support/plugin/product-countdown-for-woocommerce/reviews/?filter=5#new-post'); ?>" target="_blank"><?php echo _e('Okay You Deserve It', PCFWC_TEXTDOMAIN); ?></a>
							<a class="prowc_product_countdown_button button action" href="?prowc-wc-pc-plugin-nopemaybelater"><?php echo _e('Nope Maybe later', PCFWC_TEXTDOMAIN); ?></a>
							<a class="prowc_product_countdown_button button action" href="?prowc-wc-pc-plugin-alreadydid"><?php echo _e('I Already Did', PCFWC_TEXTDOMAIN); ?></a>
						</div>
					</div>
				</div>
			</div>
	...
	function prowc_product_countdown_plugin_notice_review() {
		$curDate = date('Y-m-d', strtotime(' + 7 days'));
		if (isset($_GET['prowc-wc-pc-plugin-nopemaybelater'])) {
