...
class ProWC_Product_Countdown_Metaboxes {
	...
	function add_counter_metabox() {
		add_meta_box(
			...
			__( 'Product Time Countdown', PCFWC_TEXTDOMAIN ),
	...
	function display_counter_metabox() {
		...
		$field_html .= '<option value="no" '  . selected( 'no',  $is_enabled, false ) . '>' .
			__( 'No', PCFWC_TEXTDOMAIN )  . '</option>';
		$field_html .= '<option value="yes" ' . selected( 'yes', $is_enabled, false ) . '>' .
			__( 'Yes', PCFWC_TEXTDOMAIN ) . '</option>';
		$field_html .= '</select>';
		$table_data[] = array( __( 'Enabled', PCFWC_TEXTDOMAIN ), $field_html );

		$field_html = '';
		$field_html .= '<input type="date" name="prowc_product_countdown_date" value="' . $countdown_date . '">';
		$table_data[] = array( __( 'Date', PCFWC_TEXTDOMAIN ), $field_html );

		$field_html = '';
		$field_html .= '<input type="time" name="prowc_product_countdown_time" value="' . $countdown_time . '">';
		$table_data[] = array( __( 'Time', PCFWC_TEXTDOMAIN ), $field_html );

		$action_options = apply_filters( 'prowc_product_countdown', array(
			'do_nothing'        => __( 'Do nothing', PCFWC_TEXTDOMAIN ),
			'disable_product'   => __( 'Disable product', PCFWC_TEXTDOMAIN ),
		), 'actions' );
		$field_html = '';
		...
		$help_tip = ( '' != ( $help_tip = apply_filters( 'prowc_product_countdown',
			__( 'Pro version also includes "Cancel sale" and "Make sold out" actions.', PCFWC_TEXTDOMAIN ), 'settings' ) ) ? wc_help_tip( $help_tip, true ) : '' );
		$table_data[] = array( __( 'Action', PCFWC_TEXTDOMAIN ) . $help_tip, $field_html );

		$html = '';
		$html .= $this->get_table_html( $table_data, array( 'table_heading_type' => 'vertical', 'table_class' => 'widefat striped' ) );
		$html .= '<p><em>' . __( 'Current date and time', PCFWC_TEXTDOMAIN ) . ': ' . current_time( 'mysql' ) . '</em></p>';
